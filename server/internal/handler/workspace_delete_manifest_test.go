...
const (
	...
	// Evidence tables retain their workspace-scoped rows after workspace
	// teardown. They intentionally carry workspace_id so audits and response
	// lineage remain attributable without being part of the delete graph.
	workspaceDeleteKeepScoped workspaceDeleteAction = "keep_scoped"
...
var workspaceDeletionManifest = map[string]workspaceDeleteAction{
	...
	"strikeflow_connector_audit":                 workspaceDeleteKeepScoped,
	"strikeflow_connector_content_reply_receipt": workspaceDeleteKeepScoped,
	"strikeflow_response_outbox":                 workspaceDeleteKeepScoped,
	"strikeflow_connector_reply_receipt":         workspaceDelete,
	"strikeflow_connector_token":                 workspaceDelete,
...
`)
	...
	for table, action := range workspaceDeletionManifest {
		...
		case workspaceDeleteKeepScoped:
			if !hasWorkspaceID {
				t.Errorf("%s table %s lost workspace_id; update its preservation selector", action, table)
			}

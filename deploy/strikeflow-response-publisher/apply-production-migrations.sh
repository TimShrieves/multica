...
test -d "$release_dir/server/migrations"
...
docker exec -i multica-postgres-1 sh -c \
  ...
  >"$evidence_dir/migration-ledger.actual-before"
cmp "$evidence_dir/migration-ledger.expected-before" "$evidence_dir/migration-ledger.actual-before"
...
SELECT 'response_migrations|' || string_agg(version,',' ORDER BY version) FROM schema_migrations WHERE version IN (
...
SQL
grep -Fx 'response_migrations|253_strikeflow_response_outbox,254_strikeflow_connector_reply_command_unique,255_strikeflow_response_outbox_event_unique,256_strikeflow_response_outbox_due_index,257_strikeflow_response_outbox_event_id_unique' "$evidence_dir/database.before"
grep -Fx 'outbox_exists|true' "$evidence_dir/database.before"
grep -Fx 'command_column_exists|true' "$evidence_dir/database.before"
...
SQL
  docker exec -i multica-postgres-1 sh -c \
    'psql -X -A -t -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT version FROM schema_migrations ORDER BY version"' \
    >"$evidence_dir/migration-ledger.failure-final" 2>&1 || true
...
docker exec -i multica-postgres-1 sh -c \
  'psql -X -A -t -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "SELECT version FROM schema_migrations ORDER BY version"' \
  >"$evidence_dir/migration-ledger.actual-after"
cmp "$evidence_dir/migration-ledger.expected-after" "$evidence_dir/migration-ledger.actual-after"
for container in multica-backend-1 multica-frontend-1 multica-postgres-1; do

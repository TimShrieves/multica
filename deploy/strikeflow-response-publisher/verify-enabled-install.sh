...
BEGIN
  ...
  FROM schema_migrations WHERE version IN (
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='strikeflow_connector_token' AND column_name='agent_id' AND data_type='uuid' AND is_nullable='YES') THEN
    RAISE EXCEPTION 'content connector agent binding column mismatch';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='strikeflow_connector_audit' AND column_name='root_comment_id' AND data_type='uuid' AND is_nullable='YES') THEN
    RAISE EXCEPTION 'content connector audit root column mismatch';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid='public.strikeflow_connector_token'::regclass
      AND conname='strikeflow_connector_token_scopes_check' AND contype='c' AND convalidated
      AND pg_get_constraintdef(oid) LIKE '%content:reply%'
  ) OR NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid='public.strikeflow_connector_token'::regclass
      AND conname='strikeflow_connector_token_content_reply_scope_check' AND contype='c' AND convalidated
      AND pg_get_constraintdef(oid) LIKE '%cardinality(scopes) = 1%'
      AND pg_get_constraintdef(oid) LIKE '%cardinality(project_ids) = 1%'
      AND pg_get_constraintdef(oid) LIKE '%agent_id IS NOT NULL%'
  ) THEN RAISE EXCEPTION 'content connector purpose-scope constraint mismatch'; END IF;
  IF to_regclass('public.strikeflow_connector_content_reply_receipt') IS NULL THEN
    RAISE EXCEPTION 'content reply receipt table absent';
  END IF;
  IF (SELECT array_agg(format('%s:%s:%s',column_name,data_type,is_nullable) ORDER BY ordinal_position) FROM information_schema.columns WHERE table_schema='public' AND table_name='strikeflow_connector_content_reply_receipt') IS DISTINCT FROM ARRAY[
    'token_id:uuid:NO','workspace_id:uuid:NO','recipient_id:uuid:NO','agent_id:uuid:NO',
    'idempotency_key:uuid:NO','issue_id:uuid:NO','root_comment_id:uuid:NO','reply_root_hash:text:NO',
    'package_id:uuid:NO','package_payload_hash:text:NO','source_revision:integer:NO','payload_hash:text:NO',
    'comment_id:uuid:YES','continuation_task_id:uuid:YES','created_at:timestamp with time zone:NO',
    'committed_at:timestamp with time zone:YES'
  ]::text[] THEN RAISE EXCEPTION 'content reply receipt column ledger mismatch'; END IF;
  IF EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='f') THEN
    RAISE EXCEPTION 'content reply receipt must not contain foreign keys';
  END IF;
  IF (SELECT count(*) FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND convalidated) <> 5
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%source_revision > 0%')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%length(reply_root_hash) = 64%')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%length(package_payload_hash) = 64%')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%length(payload_hash) = 64%')
     OR NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.strikeflow_connector_content_reply_receipt'::regclass AND contype='c' AND pg_get_constraintdef(oid) LIKE '%comment_id IS NULL%' AND pg_get_constraintdef(oid) LIKE '%committed_at IS NOT NULL%')
  THEN RAISE EXCEPTION 'content reply receipt check constraint mismatch'; END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_index
    WHERE indexrelid='public.idx_strikeflow_content_reply_receipt_unique'::regclass
      AND indrelid='public.strikeflow_connector_content_reply_receipt'::regclass
      AND indisunique AND indisvalid AND indisready AND indislive
      AND indnkeyatts=4 AND indexprs IS NULL AND indpred IS NULL
      AND pg_get_indexdef(indexrelid,1,true)='workspace_id'
      AND pg_get_indexdef(indexrelid,2,true)='recipient_id'
      AND pg_get_indexdef(indexrelid,3,true)='agent_id'
      AND pg_get_indexdef(indexrelid,4,true)='idempotency_key'
  ) THEN RAISE EXCEPTION 'content reply receipt unique index mismatch'; END IF;
